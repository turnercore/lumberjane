...
// import type { ExpirationRestriction, JwtToken, UUID } from '@/types';
...
//       const { data: decryptedKeyData, error: fetchKeyError } = await fetchAndDecryptKey(decodedToken.info.user, decodedToken.info.key)
//       if (fetchKeyError) {
//         errors.push(fetchKeyError);
//         return NextResponse.json({ errors }, { status: 500 });
//       }
//       decryptedKey = decryptedKeyData;
//     }
...
// async function fetchAndDecryptKey(userId: UUID | null = null, keyId: UUID | null = null, passphrase:string | null = null): Promise<{ data: string, error: string }> {
//   const supabase = createClient(supabaseUrl, supabaseServiceKey);
//   let error: string = '';
//   let data: string = '';

//   if (!userId || !keyId) {
//     error = 'User ID and key ID are required to fetch key from supabase.';
//     return { data, error };
//   }
...
//   if (isSecret) {
//     if (!passphrase) {
//       error = 'Passphrase is required to decrypt API key as it is double encrypted.';
//       return { data, error };
//     }
//   }
